...
def spiral_cold_spray(
    ...
    *,
    ...
    """creates the spiral and execute with servoj.

    Monotone-tilt around the circle
    - Tilt is tied to global angular progress (total φ traversed), not just step index.
    - This makes sure that as we sweep around, the tilt never increases, even after skips.
    - We also make sure that theres monotonic decrease with a small epsilon guard.
    """
